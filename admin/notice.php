...
function lmt_rating_admin_notice() {
    ...
    $dismiss = wp_nonce_url( add_query_arg( 'lmt_rating_notice_action', 'lmt_dismiss_rating_true' ), 'lmt_dismiss_rating_true' ); 
    $no_thanks = wp_nonce_url( add_query_arg( 'lmt_rating_notice_action', 'lmt_no_thanks_rating_true' ), 'lmt_no_thanks_rating_true' ); ?>
    ...
    <div class="notice notice-success">
        ...
        <a href="https://wordpress.org/support/plugin/wp-last-modified-info/reviews/?filter=5#new-post" target="_blank" class="button button-secondary"><?php _e( 'Ok, you deserve it', 'wp-last-modified-info' ); ?></a>&nbsp;
...
function lmt_dismiss_rating_admin_notice() {
    ...
    if ( 'lmt_dismiss_rating_true' === $_GET['lmt_rating_notice_action'] ) {
        check_admin_referer( 'lmt_dismiss_rating_true' );
        update_option( 'lmt_plugin_dismiss_rating_notice', '1' );
    }

    if ( 'lmt_no_thanks_rating_true' === $_GET['lmt_rating_notice_action'] ) {
